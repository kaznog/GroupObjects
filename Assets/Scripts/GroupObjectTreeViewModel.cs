...
public class GroupObjectTreeViewModel<T> where T : GroupObjectTreeViewInfomation
{
    IList<T> m_Data;
    T m_Root;
    int m_MaxID;
    ...
    public GroupObjectTreeViewModel(IList<T> data)
    {
        SetData(data);
    }
    ...
    public IList<int> GetAncestors(int id)
    {
        ...
        GroupObjectTreeViewInfomation T = Find(id);
        if (T != null)
        {
            while (T.parent != null)
            {
    ...
    public IList<int> GetParentsBelowStackBased(GroupObjectTreeViewInfomation searchFromThis)
    {
        Stack<GroupObjectTreeViewInfomation> stack = new Stack<GroupObjectTreeViewInfomation>();
        stack.Push(searchFromThis);

        var parentBelow = new List<int>();
        while (stack.Count > 0)
        {
            GroupObjectTreeViewInfomation current = stack.Pop();
    ...
    public void AddElement(T element, GroupObjectTreeViewInfomation parent, int insertPosition)
    {
        if (element == null)
        {
        ...
        {
            parent.children = new List<GroupObjectTreeViewInfomation>();
        }
    ...
    public T ListToTree<T>(IList<T> list) where T : GroupObjectTreeViewInfomation
    {
        // Validate input
        ...
        for (int parentIndex = 0; parentIndex < list.Count; parentIndex++)
        {
            ...
            // Fill child array
            List<GroupObjectTreeViewInfomation> childList = null;
            if (childCount != 0)
            {
                childList = new List<GroupObjectTreeViewInfomation>(childCount);    // Allocate one
                childCount = 0;
                for (int i = parentIndex + 1; i < list.Count; i++)
                {
                    if (list[i].depth == parentDepth + 1)
    ...
    public void TreeToList<T>(T root, IList<T> result) where T : GroupObjectTreeViewInfomation
    {
        if (result == null)
        {
    ...
    public void ValidateDepthValues<T>(IList<T> list) where T : GroupObjectTreeViewInfomation
    {
        if (list.Count == 0)
        {
    ...
    public IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements) where T : GroupObjectTreeViewInfomation
    {
        if (elements.Count == 1)
    ...
    public void UpdateDepthValues<T>(T root) where T : GroupObjectTreeViewInfomation
    {
        if (root == null)
        {
    ...
    public bool IsChildOf<T>(T child, IList<T> elements) where T : GroupObjectTreeViewInfomation
    {
        while (child != null)
        {
